...
export default function TradingJournal() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try { const r = await window.storage.get("tj_positions", false); if (r) pos = JSON.parse(r.value); } catch (e) {}
      try { const r = await window.storage.get("tj_balance_ops", false); if (r) bal = JSON.parse(r.value); } catch (e) {}
      try { const r = await window.storage.get("tj_account_meta", false); if (r) meta = JSON.parse(r.value); } catch (e) {}
      try { const r = await window.storage.get("tj_last_updated", false); if (r) lu = r.value; } catch (e) {}
      if (mounted) {
  ...
  async function handleFiles(fileList) {
    ...
    setLastUpdated(now);
    try {
      await window.storage.set("tj_positions", JSON.stringify(mergedPositions), false);
      await window.storage.set("tj_balance_ops", JSON.stringify(mergedBalanceOps), false);
      if (newMeta) await window.storage.set("tj_account_meta", JSON.stringify(newMeta), false);
      await window.storage.set("tj_last_updated", now, false);
    } catch (e) {}
    if (failed.length) {
  ...
  async function handleReset() {
    ...
    try {
      await window.storage.delete("tj_positions", false);
      await window.storage.delete("tj_balance_ops", false);
      await window.storage.delete("tj_account_meta", false);
      await window.storage.delete("tj_last_updated", false);
    } catch (e) {}
  }
  ...
  return (
    <div style={{ background: C.bg, color: C.text, minHeight: 480 }} className="rounded-2xl p-6 md:p-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        ...
        <StatCard icon={AlertTriangle} label="Max drawdown" value={fmtPct(a.maxDrawdownPct)} tone={a.maxDrawdownPct > 50 ? "bad" : undefined} sub={`${a.daysTracked} active days`} />
      </div>
